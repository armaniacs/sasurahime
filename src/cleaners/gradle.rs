...
impl Cleaner for GradleCleaner {
    ...
    fn clean(&self, dry_run: bool) -> Result<CleanResult> {
        ...
        for path in &old {
            ...
            if dry_run {
                println!("[dry-run] [gradle] would remove: {} ({})",
                    path.display(), crate::format::format_bytes(size));
            } else {
                self.runner.run("chflags", &["-R", "nouchg", &path.to_string_lossy()]).ok();
...
impl JetBrainsCleaner {
    ...
    fn find_old_caches(jetbrains_dir: &Path) -> Vec<PathBuf> {
        ...
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            let ide_name: String = name.chars().take_while(|c| c.is_ascii_alphabetic()).collect();
            if ide_name.is_empty() { continue; }
            let key: Vec<u32> = name[ide_name.len()..]
                .split(|c: char| !c.is_ascii_digit())
                .filter_map(|s| s.parse().ok())
                .collect();
            if key.is_empty() { continue; }
            by_ide.entry(ide_name).or_default().push((key, entry.path()));
        }
...
impl Cleaner for JetBrainsCleaner {
    fn name(&self) -> &'static str { "jetbrains" }
    ...
    fn clean(&self, dry_run: bool) -> Result<CleanResult> {
        ...
        for path in &old {
            let size = dir_size(path);
            if dry_run {
                println!("[dry-run] [jetbrains] would remove: {} ({})",
                    path.display(), crate::format::format_bytes(size));
            } else {
                self.runner.run("chflags", &["-R", "nouchg", &path.to_string_lossy()]).ok();
                fs::remove_dir_all(path)?;
