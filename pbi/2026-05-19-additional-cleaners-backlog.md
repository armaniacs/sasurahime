...
```
中優先度（実装推奨順）:
1. ollama          ← サイズ最大、モデル削除の誤操作リスクを考慮した設計が重要
2. simulator       ← xcrun simctl 一発で安全に削除できる
3. xcode-device-support ← 世代管理ロジックが必要で実装コスト中程度
4. maven           ← Java 開発者向け、GenericCleaner パターンで容易
5. terraform       ← 同上
6. flutter         ← 同上

低優先度（保留）:
- volta / sbt     → ユーザー要望があれば対応
- tree-sitter     → editor-caches としてまとめて実装
- vscode-logs     → PBI-007 の拡張として実装
```
