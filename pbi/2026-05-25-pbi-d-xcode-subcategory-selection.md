...
**Phase 2 (TUI 展開): ⬜ 未着手**
- `XcodeCleaner::sub_targets()` オーバーライド（現在デフォルトの `vec![]` を返す）
- TUI でのサブカテゴリチェックボックス展開表示
- 存在しないサブカテゴリの無効化表示
...
- [ ] DerivedData / Archives の各サイズを個別に表示する（TUI）
- [ ] 任意の組み合わせを選択して削除できる（TUI）
...
TUI テスト（未実装）:
- interactive で sub_targets() が空でない場合の展開表示テスト
- サブカテゴリ選択→削除の一連の動作テスト
```
...
Phase 2 (未着手):
1. Red: `sub_targets()` の統合テストを作成
2. Green: `XcodeCleaner::sub_targets()` をオーバーライド
3. Refactor: TUI のチェックボックス展開ロジックを `SubcategorySelector` に抽出（任意）
...
Phase 2 (TUI):
- [ ] TUI で Xcode 選択時に DerivedData / Archives が展開表示される
- [ ] 各サブカテゴリのサイズが表示される
- [ ] 任意の組み合わせが選択可能
- [ ] コードレビュー完了
